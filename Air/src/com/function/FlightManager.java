...
import com.thread.GetFindOneResult;
import com.thread.GetInsertResult;
import com.thread.GetModifyFindResult;
import com.thread.GetModifyResult;
...
public class FlightManager implements ActionListener, ItemListener {
	...
	static JLabel jLabel, jl_flightNum, jl_leaveCity, jl_arriveCity,
			jl_leaveDate, jl_price;
	static JTextField jtf_flightNum, jtf_leaveCity, jtf_arriveCity,
			jtf_leaveDate, jtf_price, jtf_delete, jtf_modify, jtf_find;
	ButtonGroup bg_find;
	JRadioButton rb_findOne, rb_findAll;
	Map<String, Object> map;
	public static List<Map<String, Flight>> list = null;
	...
	public void cancel() {
		if (jp_insert != null)
			Function.frame.remove(jp_insert);
		if (jp_delete != null)
			Function.frame.remove(jp_delete);
		if (jp_modify != null)
			Function.frame.remove(jp_modify);
		if (jp_find != null)
			Function.frame.remove(jp_find);
		if (jp_one != null)
			Function.frame.remove(jp_one);
		if (jp_all != null)
			Function.frame.remove(jp_all);
	...
	public void insertFight() {
		...
		Function.frame.getContentPane().add(jp_insert);
		...
		insert_ensure.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				new GetInsertResult(new Flight(jtf_flightNum.getText(),
						jtf_leaveCity.getText(), jtf_arriveCity.getText(),
						jtf_leaveDate.getText(), Integer.parseInt(jtf_price
								.getText())), Function.frame).start();
			}
		});
	...
	public void deleteFight() {
		...
		JLabel jLabel = new JLabel("请输入想要撤销的航班编号：");
		...
		delete_ensure.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent e) {
				new GetDeleteResult(jtf_delete.getText(), 3,
	...
	public void modifyFight() {
		...
		Function.frame.getContentPane().add(jp_modify);
		jp_modify.setLayout(null);

		jLabel = new JLabel("请输入想要修改的航班编号：");
		...
		jtf_modify.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				new GetModifyFindResult(str, 3, Function.frame).start();
			}
		});
		...
		modify_ensure.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent e) {
				new GetModifyResult(new Flight(jtf_flightNum.getText(),
						jtf_leaveCity.getText(), jtf_arriveCity.getText(),
						jtf_leaveDate.getText(), Integer.parseInt(jtf_price
								.getText())), Function.frame).start();
			}
	...
	public static void change(Flight flight) {
	...
	public void findFight() {
		...
		Function.frame.getContentPane().add(jp_find);
		...
		find_ensure.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				if (rb_findAll.isSelected()) {
					new GetFindAllResult(3, Function.frame).start();
				} else if (rb_findOne.isSelected()) {
					new GetFindOneResult(jtf_find.getText(), 3,Function.frame).start();
				}
			}
		});
		cancel.addActionListener(this);
	...
	public static void showAll() {
		...
		Function.frame.getContentPane().add(jp_all);
		...
		for (int i = 0; i < list.size(); i++) {
			Flight flight = (Flight) list.get(i).get("flight");
	...
	public static void showOne(String flightNum, Flight flight) {
		if (jp_insert != null)
			Function.frame.remove(jp_insert);
		if (jp_delete != null)
			Function.frame.remove(jp_delete);
		...
		Function.frame.getContentPane().add(jp_one);
		...
		if (GetFindOneResult.getInfo) {
			flight = GetFindOneResult.flightMap.get("flight");
