...
public class GetTicketInsertResult extends Thread {
	...
	public GetTicketInsertResult(User user, JFrame jFrame) {
		this.user = user;
		this.jFrame = jFrame;
		flag = 1;
	}

	public GetTicketInsertResult(Customer customer, JFrame jFrame) {
		this.customer = customer;
		this.jFrame = jFrame;
		flag = 2;
	}

	public GetTicketInsertResult(Flight flight, JFrame jFrame) {
		this.flight = flight;
		this.jFrame = jFrame;
		flag = 3;
	}

	public GetTicketInsertResult(Order order, JFrame jFrame) {
		this.order = order;
		this.jFrame = jFrame;
		flag = 4;
	}
	...
	public void run() {
		...
		case 2:
			...
			if (map.get("result").equals("1")) {
				new GetTicketInsertResult(TicketManager.order, this.jFrame).start();
			} else {
